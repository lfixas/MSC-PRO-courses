...
def new_game():
    global game_infos, guess_box, guess_input, picture_box, info_result, guess, past_guess, match_guess, points, guess_word, difficulty_value, full_guess, random_word, resultbox, timer_running
    guess = ""
    past_guess = ""
    guess_input = ""
    guess_box = ""
...
def refresh():
    ...
    if info_result == "Win":
        resultbox.configure(text="Congratulations !", fg_color="lime", font=customtkinter.CTkFont(size=16, weight="bold"))
    if info_result == "Lose":
        resultbox.configure(text="End of the game, you are hanged!", fg_color="red", font=customtkinter.CTkFont(size=16, weight="bold"))
...
button_width = 30  # Adjust key button
